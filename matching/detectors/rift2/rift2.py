...
class RIFT2:
    def __init__(self, config_file=None, **external_params):
        ...
        self.default_config = {
            ...
            'npt': 4096,  # Number of keypoints to retain after feature detection
    ...
    def feature_description(self, img_hw, eo, kpts):
        ...
        des = Parallel(n_jobs=2)(delayed(process_keypoint)(k) for k in range(n))
        #des = [process_keypoint(k) for k in range(n)]
        des = np.array(des).T
    ...
    def __call__(self, im1: torch.Tensor, im2: torch.Tensor):
        ...
        kp1, des1, kp2, des2 = self.process_features(im1, im2)
        if not kp1 or not kp2:
            return None
        mp1, mp2, _ = match_keypoints_nn(des1, des2, kp1, kp2, self.config['lowes_ratio'], False)

        p = (2 * len(mp1)) / (len(kp1) + len(kp2))

        mp1 = mp1[:, ::-1]
        mp2 = mp2[:, ::-1]

        return DetectedPoints(mp1, mp2, p)

...
from matching.roma.utils import get_tuple_transform_ops
from matching.roma.utils.local_correlation import local_correlation
from matching.roma.utils.utils import cls_to_flow_refine
from matching.roma.utils.kde import kde
...
class ConvRefiner(nn.Module):
    ...
    def forward(self, x, y, flow, scale_factor = 1, logits = None):
        ...
        with torch.autocast("cuda", enabled=self.amp, dtype = self.amp_dtype):
            with torch.no_grad():
                x_hat = F.grid_sample(y.float(), flow.permute(0, 2, 3, 1).float(), align_corners=False, mode = self.sample_mode)
...
class RegressionMatcher(nn.Module):
    ...
    def recrop_from_path(self, certainty, image_path):
        roi = self.get_roi(certainty, *Image.open(image_path).size)
        return Image.open(image_path).convert("RGB").crop(roi)

    def recrop_image(self, certainty, image: np.ndarray):
        l, u, r, b = self.get_roi(certainty, *image.shape[:2][::-1])
        return image[u:b, l:r]

    @torch.inference_mode
    def match_arrays(self, im_A: np.ndarray, im_B: np.ndarray,device,
            normalize=False, scale=1/255.):
        ...
        with torch.no_grad():
            ...
            test_transform = get_tuple_transform_ops(
                resize=(hs, ws), normalize=normalize, clahe=False, scale=scale
            )
            ...
            if self.upsample_preds:
                ...
                torch.cuda.empty_cache()
                test_transform = get_tuple_transform_ops(
                    resize=(hs, ws), normalize=normalize, scale=scale
                )
                if self.recrop_upsample:
                    certainty = corresps[finest_scale]["certainty"]
                    print(certainty.shape)
                    im_A = self.recrop_image(certainty[0, 0], im_A_copy)
                    im_B = self.recrop_image(certainty[1, 0], im_B_copy)
                    # TODO: need to adjust corresps when doing this
                else:
                    im_A, im_B = im_A_copy, im_B_copy
                im_A, im_B = test_transform((im_A, im_B))
        ...
        with torch.no_grad():
            ...
            if self.upsample_preds:
                ...
                if self.recrop_upsample:
                    certainty = corresps[finest_scale]["certainty"]
                    print(certainty.shape)
                    im_A = self.recrop_from_path(certainty[0,0], im_A_path)
                    im_B = self.recrop_from_path(certainty[1,0], im_B_path)
                    #TODO: need to adjust corresps when doing this
                else:
                    im_A, im_B = im_A_copy, im_B_copy
                im_A, im_B = test_transform((im_A, im_B))
    ...
    def visualize_warp(self, warp, certainty, im_A = None, im_B = None, 
        ...
        if save_path is not None:
            from matching.roma.utils import tensor_to_pil
